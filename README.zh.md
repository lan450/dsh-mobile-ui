...
| 输入区安全区 | 输入区避开 iOS 底部安全区；模型触发键缩写成 Flash/Pro |
...
桌面布局不受影响。计划审批/选项卡片采用两行 footer，操作按钮在任何宽度下都完整可见。
...
派生类名失配时该块会走结构选择器兜底（手机端至少可用），或直接输出空
（如 modelSheet/popupSheet 无兜底 CSS），此时 `notes` 会给出原因。
...
- 所有覆盖样式都在 `@media (max-width: 820px)` 内，桌面端不受影响。
- 哈希类名在运行时从官方组件样式表
