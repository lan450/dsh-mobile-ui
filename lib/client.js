 ...
 *
 *  v0.3.19：
 *  20. 修掉「对话/轨迹」标签行右端多出的小白条（会话态压在鲸鱼上那条）。
 *      根因三段叠加：① 官方 0.1.5-rc.1 把激活 tab 下划线改成
 *      `.tab::after{height:2px;bottom:-1px}`（0.1.1 是 padding-bottom:11px +
 *      bottom:1px，下划线在盒内），于是标签行内容比行内容盒低 1px；
 *      ② 本插件 chatLayout 给 .tabs 开了 `overflow-x:auto`，按 CSS 规范
 *      另一轴的 visible 会计算成 auto → 标签行成了两轴滚动容器，这 1px
 *      溢出就长出一条竖向滚动条；③ 官方全局 `::-webkit-scrollbar{width:8px}`
 *      + thumb `#e5e5e5` 圆角，安卓 Chromium 对自定义滚动条用常驻（非 overlay）
 *      模式渲染 → 标签行右端出现 8×23px 浅色小条；而标签行官方是
 *      position:relative; z-index:1，手机端鲸鱼 z-index:0，滚动条画在鲸鱼之上，
 *      会话态鲸鱼又贴顶到右上角，于是看着像鲸鱼图标右边多了个白条。
 *      修法：行底补 1px padding（+ 负 1px margin 抵消高度变化）让下划线落进
 *      行内 padding 区，溢出归零、官方下划线 2px 完整且位置不变；竖向显式
 *      hidden 兜底（官方以后再改下划线偏移也不会再长出滚动条）；横向 auto
 *      保留但隐藏滚动条（`scrollbar-width:none` + `::-webkit-scrollbar{display:none}`，
 *      与官方 .stripTabs 同一写法），标签多时照旧可以横滑，只是不再画出滚动条。
 *
...
`;
	...
	/* 「对话/轨迹」标签行：只允许横向滑动，绝不再长出竖向滚动条。
	   官方 0.1.5-rc.1 的激活下划线是 .tab::after{height:2px;bottom:-1px}
	   （0.1.1 还是 bottom:1px），比行内容盒低 1px；本行又开了
	   overflow-x:auto（按规范 overflow-y 的 visible 会计算成 auto）→ 这 1px
	   变成一条竖向滚动条，安卓 Chromium 对自定义滚动条常驻渲染
	   （8px、thumb #e5e5e5 圆角），正好压在会话态贴顶的鲸鱼上。
	   ① padding-bottom 1px：给下划线留出行内空间（溢出归零，下划线仍 2px
	      且位置不变）；margin-bottom -1px 抵消这 1px，头部总高不变。
	   ② overflow-y: hidden：即便官方以后再让下划线多探出几像素，也只是裁掉
	      超出部分，不会再出现滚动条。
	   ③ 横向 auto 保留 + 滚动条隐藏（同官方 .stripTabs 的 scrollbar-width:
	      none 写法）：标签多时仍可横滑，但不再画出一条滚动条。 */
	.${c.tabs} { gap: 18px !important; padding-left: 4px !important; padding-bottom: 1px !important; margin-bottom: -1px !important; overflow-x: auto !important; overflow-y: hidden !important; scrollbar-width: none !important; margin-top: 10px !important; }
	.${c.tabs}::-webkit-scrollbar { display: none !important; }
