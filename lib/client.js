 ...
 *
 *  v0.3.13：
 *  16. 修复会话页头部「行距被撑大」：原 chatLayout 用 flex-wrap +
 *      crumbs(flex:1 1 auto)，标题面包屑（含「N 个子代理」）填满整行后，
 *      headerUtilities（下载会话）被挤到中间独立一行，标题行与动作行之间
 *      多出一行、行距被撑大（桌面端正常，手机 ≤820px 明显）。改为两列
 *      grid：col1 = 标题面包屑（minmax(0,1fr) 可截断收缩），col2 = 下载
 *      会话（auto，justify-self:end 始终靠右锚定）；标准模式+后台任务仍
 *      独占第二行——头部稳定回到「任务名称 + 下载会话」一行 / 「标准
 *      模式 + 后台任务」一行的两行布局，任何标题长度下下载按钮都不会被
 *      挤到中间行。
 *
...
`;
	...
	/* 手机上头部两行固定结构：
	   第一行 = 标题面包屑（任务名称，可收缩省略号）… 下载会话（最右，
	   永远不被挤下去）; 第二行 = 标准模式 + 后台任务（宽度受限，超长省略号）。
	   改为两列 grid：col1 = crumbs（minmax 0 → 可截断收缩），col2 = 下载
	   按钮（auto，靠右）。grid 固定列宽，标题不管多长 / flex 都不会把
	   下载按钮挤到中间行（旧 flex-wrap + crumbs flex:1 1 auto 会让
	   标题+“2 个子代理”填满整行、把 headerUtilities 挤到下一行，
	   于是标题行和动作行之间多出一行、行距被撑大）。
	...
	/* 任务名称（面包屑）固定在 col1/row1，min-width:0 + overflow 截断 */
	.${c.crumbs} { grid-column: 1 !important; grid-row: 1 !important; min-width: 0 !important; }
	/* 下载会话固定在 col2/row1、靠右锚定，永不掉到第二行 */
	.${c.headerUtilities} { grid-column: 2 !important; grid-row: 1 !important; justify-self: end !important; margin-left: 0 !important; }
	/* 标准模式 + 后台任务独占第二行 */
	.${c.headerActions} { grid-column: 1 / -1 !important; grid-row: 2 !important; min-width: 0 !important; flex-wrap: wrap !important; }
