...
`,
			},
			{
				...
				entries: ["frame", "sidebarCol", "detailsCol", "handle"],
				build: (c) => {
	...
	${F}:not([data-sidebar-collapsed]) { grid-template-columns: 0px minmax(0, 1fr) 0px !important; }
...
`;
			...
			},
			{
				...
				type: "static",
				css: `
	/* 输入区避开 iOS 底部安全区；模型触发键已缩写成 Flash/Pro，输入区
	   恢复原生单行布局。v3.1 起不再派生 InputBar 类名——该模块只有
...
`,
	...
	/* 触发器只显示模型名（Flash/Pro 由 modelLabel 块缩写），推理等级在抽屉里选 */
...
`;
		...
		// ── 模型触发键缩写：只显示 Flash / Pro ────────────────────────────
		// 模型目录只有 deepseek-v4-flash / deepseek-v4-pro 两个模型，但触发器
		// 显示全名 + 推理等级（"DeepSeek-V4-Flash · High"）太长。这里把可见
		// 文本缩写为 Flash/Pro，并隐藏推理等级徽标（推理等级仍可在抽屉里选）。
		function setupModelLabelShortener() {
			...
			const shorten = () => {
				...
				for (const el of document.querySelectorAll(`.${labelCls}`)) {
					...
					let short = text;
					if (/flash/i.test(text)) short = "Flash";
					else if (/pro/i.test(text)) short = "Pro";
					// 幂等：React 原地重渲染会改回全名，这里每次发现全名就再缩写。
					// 不用 dataset 标记——标记会让重渲染后的全名永远被跳过。
					if (short !== text) el.textContent = short;
				}
			};
			if (ensure()) shorten();
		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				const mq = window.matchMedia(MOBILE_QUERY);
				const syncMobile = () => document.documentElement.classList.toggle("dsh-mobile", mq.matches);
				syncMobile();
				...
				console.info(
					`[dsh-mobile-ui] 已加载 · 视口 ${window.innerWidth}px · 移动端查询命中: ${mq.matches} · ` +
