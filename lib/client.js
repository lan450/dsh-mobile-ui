...
`;
			...
			},
			{
				...
				entries: ["bar", "actions", "goalGlyph"],
				build: (c) => {
					if (!c) return "";
					return `
	/* 目标栏（"进行中的目标" + 暂停/编辑/废弃）：官方 bar 右内边距只有
	   5px（padding: 4px 5px 4px 12px，左 12 右 5 不对称）。桌面端栏本身
	   居中收窄、视觉无感；手机上整条栏贴近屏右缘（实测栏右缘距视口仅
	   32px），最后面的废弃按钮距栏右缘只剩 5px，看起来贴边。移动端把
	   右内边距加大到 12px（与左内边距对称），按钮离栏缘/屏缘恢复呼吸感；
	   按钮间距 10px→8px 微收，≤374px 的极窄屏（如 360px 安卓）再隐藏
	   装饰性目标图标（goalGlyph），让固定内容腾出空间、留白不缩水。 */
