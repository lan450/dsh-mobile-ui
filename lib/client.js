 ...
 *
 *  v0.3.15：
 *  18. 修复「内测模型 deepseek-v4.1-flash-expires-on-0910（配置名 4.1 Flash，
 *      报障时叫 v4.1-Flash）在触发键上被缩成 Flash、与 V4-Flash 撞名」。
 *      根因：缩写规则按官方全名（DeepSeek-V4-Flash 等）写的纯正则，第一个
 *      命中即生效，/flash/i 会命中任何含 flash 的名字；settings.yaml 里的
 *      name 早已简化成 Flash / Pro / Vision / 4.1 Flash，再缩一遍就把版本号
 *      4.1 丢掉。改为：只在「长名」（> 14 字符的官方全名/第三方目录名）上
 *      缩写，已经够短的配置名原样显示——缩写是给长名字兜底的，不再改写
 *      用户自己起的短名。
 *
...
`;
			...
			},
			{
				...
				type: "static",
				css: `
	/* 输入区避开 iOS 底部安全区；模型触发键只在长名时缩写成
	   Flash/Pro/Vision/Model（短配置名原样显示），输入区恢复原生单行布局。
...
`,
	...
	/* 官方新的「上下文用量」按钮（ContextMeter）加进底行 .trailing 后，
	   ...
	   （v0.3.15 起触发键显示配置名，实测 4.1 Flash 标签 51px、
	   长名缩写后 ≤ Vision 的 35px；320/340/360/390px 四档底行高度
	   恒 40px、无溢出，再长由触发键自身的 max-width + ellipsis 兜底。） */
...
`;
	...
	/* 触发器只显示模型名（短名原样、长名由 modelLabel 块缩成 Flash/Pro/Vision/Model），推理等级在抽屉里选 */
...
`;
		...
		// ── 模型触发键缩写：长名兜底缩成 Flash / Pro / Vision / Model ────
		// 触发键显示的是服务端下发的 model.name（settings.yaml 的 name:）。官方
		// 全名（"DeepSeek-V4-Flash-Vision-Exp"）在手机端太长，这里按官方目录把
		// 可见文本缩短：V4-Flash → Flash，V4-Pro → Pro，V4-Flash-Vision-Exp →
		// Vision（只取视觉特征，比 "Flash Vision" 更短、窄屏底行不挤出两行）；
		// 注意先判定 vision，否则视觉模型名字里含 "flash" 会被误缩成 Flash。
		// 其它长名（含未来新增/第三方目录）缩成名量词 Model。
		//
		// v0.3.15 起只对「长名」动手（MAX_SHORT_NAME 个字符以上）：名字本身够短
		// 时原样显示。原因是 2026-09 起 settings.yaml 里的 name 已简化成
		// Flash / Pro / Vision / 4.1 Flash，再缩写一遍只会丢信息——"4.1 Flash"
		// （报障时叫 v4.1-Flash）会命中 /flash/i 被压成 "Flash"，和内测前的
		// V4-Flash 撞名，用户就看不出自己选的是哪个。缩写是给长名字兜底用的，
		// 不该改写用户自己起的短名。官方占位文案（未选模型时的 "选择模型"/
		// "Select model"）永远不动。推理等级徽标由 modelSheet 块隐藏（推理
		// 等级仍可在抽屉里选）。
		function setupModelLabelShortener() {
			...
			// 超过这个长度才算「长名」，才值得缩写。settings.yaml 里已简化的
			// 配置名（Flash 5、Pro 3、Vision 6、4.1 Flash 9）都在这条线以下，
			// 原样显示；官方全名（DeepSeek-V4-Flash 17、…-Vision-Exp 27）在
			// 线上，照旧缩写。
			// 缩写结果 Flash/Pro/Vision/Model 都远短于这条线，所以函数天然幂等，
			// 不会把自己的输出再缩一遍。
			const MAX_SHORT_NAME = 14;
			const shorten = () => {
				if (!labelCls) return;
				for (const el of document.querySelectorAll(`.${labelCls}`)) {
					const text = el.textContent || "";
					const trimmed = text.trim();
					// 官方占位文案（未选模型时）永远保持原样——它不指代任何模型。
					if (/^(选择模型|select model)$/i.test(trimmed)) continue;
					// 已经够短的配置名原样显示（见上方 v0.3.15 说明）。
					if (trimmed.length <= MAX_SHORT_NAME) continue;
					let short = text;
					...
					// 兜底：其它长名（含未来新增/第三方目录）全缩成名量词
					// "Model"，触发键永不溢出。
					else if (/[- ]/.test(trimmed)) short = "Model";
